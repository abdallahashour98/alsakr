...
import 'package:al_sakr/features/clients/presentations/client_dialog.dart'; // TODO: Move this to features/clients/presentations later
import 'package:al_sakr/models/transaction_item_model.dart';
// Temporary import for the detail screen until we split it
...
class _ClientsScreenState extends ConsumerState<ClientsScreen> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _canEdit = userRecord.data['allow_edit_clients'] ?? false;
  ...
  Future<void> _loadStaticStats() async {
    // TODO: Replace with SalesController data
    // For now, we will leave this as 0 until we connect it to the new SalesController properly
    // to avoid importing the old SalesService.
    if (mounted) {
      setState(() {
        _totalSales = 0.0;
        _totalCollected = 0.0;
  ...
  Future<void> _openClientDialog({Map<String, dynamic>? client}) async {
    ...
    await showDialog(
      ...
      builder: (ctx) => ClientDialog(
        client: client,
      ), // Assuming it still uses old logic for now or we update it next
    );
  }
...
class _ClientDetailScreenState extends ConsumerState<ClientDetailScreen> {
  ...
  Future<void> _loadDetails() async {
    ...
    final sales = await ref
        ...
        .getSalesByClient(widget.client['id']);
    final receipts = await ref
        .read(salesControllerProvider.notifier)
        .getReceiptsByClient(widget.client['id']);
    final returns = await ref
        .read(salesControllerProvider.notifier)
        .getReturnsByClient(widget.client['id']);
    final openingBal = await ref
        .read(salesControllerProvider.notifier)
        .getClientOpeningBalance(widget.client['id']);
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  final clientRec = await globalPb
                      .collection('clients')
  ...
  Future<void> _deleteReceipt(String receiptId, double amount) async {
    try {
      await globalPb.collection('receipts').delete(receiptId);
      final clientRec = await globalPb
          .collection('clients')
          .getOne(widget.client['id']);
            ...
            children: [
              ...
              FutureBuilder<List<TransactionItemModel>>(
                future: isInvoice
                    ? ref
                          .read(salesControllerProvider.notifier)
                          .getSaleItems(record['id'])
                    : ref
                          .read(salesControllerProvider.notifier)
                          .getReturnItems(record['id']),
